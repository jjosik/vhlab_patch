function [frequency_data,amplitude_data,duration_data,sampling_rate,is_E_test,acq_duration] = O_PSC_deconvolution_EDIT( plot_it,save_it,bin_sizes,num_devs_E,num_devs_I,s_margin,windowed_baseline,th_follows_base,filter_window)
...
duration_data = struct('threshold_crossing_durations',[],'fwhm_durations',[],'risetimes',[],'decay_taus',[]);
...
r_ten_values = zeros(length(collected_amps),1);          %10-90% rise time 9/28/16
r_ninety_values = zeros(length(collected_amps),1);       %10-90% rise time 9/28/16
d_tau = zeros(length(collected_amps),1);
for j=1:length(collected_amps),
    if collected_amps(j,1) == 0,
        continue
    else
        if is_E_test,
            half_max(j,1) = baseline(collected_peaks(j,1))-(abs(data_smoothed(collected_peaks(j,1)))-abs(baseline(collected_peaks(j,1))))/2;
        else
            half_max(j,1) = baseline(collected_peaks(j,1))+(abs(data_smoothed(collected_peaks(j,1)))-abs(baseline(collected_peaks(j,1))))/2;
        end
        ...
        %alt_rise_data_w = data_smoothed(alt_crossings(j):collected_peaks(j,1),1);   %won't work as is; need to confirm vector length same as crossings; might need separate loop
        %alt_decay_data_w = data_smoothed(collected_peaks(j,1):crossings(j+1,1),1);  %same; see next line above
        ...
        r_ten_values(j,1) = 0.1.*(2*(abs(half_max(j,1))));                                  %10-90% rise time
        r_ninety_values(j,1) = 0.9.*(2*(abs(half_max(j,1))));                               %10-90% rise time
         ...
         %ten_ninety_risetime(j,1) = (Rninety_ind-Rten_ind)*(1/sampling_rate);              %10-90% rise time 9/28/16
         %[d_crit_point,d_crit_ind] = min(abs((1/(exp(1))*(2*(abs(half_max(j,1)))))-decay_data_w)); %10-90% rise time 9/28/16
         %th_d_tau(j,1) = (length(decay_data_w)-d_crit_ind)*(1/sampling_rate);                      %10-90% rise time 9/28/16
        pre_fwhm(j,1) = (length(rise_data_w)-rise_ind(1,1)+(decay_ind(1,1)-1))*(1/sampling_rate);
        if any(alt_crossings<crossings(j)),
        ...
        while alt_crossings(alt_ind(1,1)+adv)<collected_peaks(j,1),
            if (alt_ind(1,1)+adv)<numel(alt_crossings),
                adv = adv+1;
            else
                break;
            end
        ...
        total_rise_data_w = [lowrise_data_w;rise_data_w];                                   %10-90% rise time
        total_decay_data_w = flipud(unique([decay_data_w;lowdecay_data_w]));                %decay time constant
        y_range_ten = 0.1*abs(total_rise_data_w(end)-total_rise_data_w(1));
        if is_E_test,
            alt_r_ten_values(j,1) = max(total_rise_data_w)-y_range_ten;
            alt_r_ninety_values(j,1) = min(total_rise_data_w)+y_range_ten;
        else
            alt_r_ten_values(j,1) = min(total_rise_data_w)+y_range_ten;
            alt_r_ninety_values(j,1) = max(total_rise_data_w)-y_range_ten;
        end
        [~,alt_Rten_ind] = min(abs(alt_r_ten_values(j,1)-total_rise_data_w));               %*current 10-90% method in effect
        [~,alt_Rninety_ind] = min(abs(alt_r_ninety_values(j,1)-total_rise_data_w));         %*current 10-90% method in effect
        %[Rnew_ten_val,Rten_ind] = min(abs(total_rise_data_w-r_ten_values(j,1)));            %10-90% rise time
        %[Rnew_ninety_val,Rninety_ind] = min(abs(total_rise_data_w-r_ninety_values(j,1)));   %10-90% rise time
        pre_ten_ninety_risetime(j,1) = (alt_Rninety_ind-alt_Rten_ind)*(1/sampling_rate);            %10-90% rise time
        d_amp = abs(total_decay_data_w(end,1)-total_decay_data_w(1,1));                     %decay time constant
        if is_E_test,                                                                       %decay time constant ('if' block)
            [new_d_crit,new_d_ind] = min(abs((total_decay_data_w(end,1)+(1/(exp(1))*d_amp))-total_decay_data_w));
        else
            [new_d_crit,new_d_ind] = min(abs((total_decay_data_w(end,1)-(1/(exp(1))*d_amp))-total_decay_data_w));
        end
        pre_d_tau(j,1) = (new_d_ind)*(1/sampling_rate);                                     %decay time constant
...
ten_ninety_risetime = nonzeros(pre_ten_ninety_risetime);                                    %10-90% rise time
d_tau = nonzeros(pre_d_tau);                                                                %decay time constant
...
%risetime histogram (more important by cell than single run)     added 9/28/16
r_binsize = 2*(prctile(ten_ninety_risetime,75)-prctile(ten_ninety_risetime,25))*(1/(nthroot(length(ten_ninety_risetime),3)));  %Freedman-Diaconis rule
%r_binsize = 0.0002;
...
D_r = D_r(1:end-1);
f4 = figure;
bar(r_bin_centers.*1000,D_r,'r');
hold on;
ylabel('Count');
xlabel('Rise time (msec)');
ylim([0 max(D_r)+0.1*max(D_r)]);
...
%decay time constant histogram (more important collected by cell rather
%than single run)
d_binsize = 2*(prctile(d_tau,75)-prctile(d_tau,25))*(1/(nthroot(length(d_tau),3)));  %Freedman-Diaconis rule
edges_d = 0:d_binsize:(max(d_tau)*1.5);
D_d = histc(d_tau,edges_d);
d_bin_centers = (edges_d(1:end-1)+edges_d(2:end))/2;
D_d = D_d(1:end-1);
f5 = figure;
bar(d_bin_centers.*1000,D_d,'b');
hold on;
ylabel('Count');
xlabel('Decay time constant (ms)');
ylim([0 max(D_d)+0.1*max(D_d)]);
title('Distribution of PSC event decay taus');
if save_it == 1,
    saveas(gcf, [pwd filesep 'event_decay_tau' '_filter_w_',num2str(filter_window),'.fig']);
    close(f5);
else
end
...
duration_data.decay_taus = d_tau;   %note to self: need to modify dependencies for this output in higher level code
